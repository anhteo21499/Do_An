...
public class ProductService {
		...
		if (!isEmptyUploadFile(productImages)) { // có upload ảnh lên.
			for (MultipartFile productImage : productImages) {
				...
				productImage.transferTo(
						new File("/home/anh/Documents/thuc_tap/ThucTap/com.nguyenquocanh/upload1/" + productImage.getOriginalFilename()));
	...
	public void save(MultipartFile productImages, KhuyenMai khuyenMai) throws IllegalStateException, IOException {

				// lưu vật lí
				productImages.transferTo(
						new File("/home/anh/Documents/thuc_tap/ThucTap/com.nguyenquocanh/upload1/" + productImages.getOriginalFilename()));
				khuyenMai.setTitle(productImages.getOriginalFilename());
		...
		if (product.getId() != null) { // chỉnh sửa
			...
			if (!isEmptyUploadFile(productImages)) { // nếu admin sửa ảnh sản phẩm
				...
				for (ProductImage _images : oldProductImages) {
					new File("D:/home/anh/Documents/thuc_tap/ThucTap/com.nguyenquocanh/upload1/" + _images.getPath()).delete();
				}
...
//		Query query = entityManager.createQuery(jpql, Product.class);
	...
	public void deleteImage(MultipartFile[] productImages, Products product) {
			// lấy danh sách ảnh cũ của sản phẩm
			List<ProductImage> oldProductImages = product.getProductImage();
			...
			// xoá ảnh trên database
			product.removeProductImagesList(oldProductImages);
